...
namespace Coherence.Runtime.Tests
{
    ...
    public class GameServersServiceTests : CoherenceTest
    {
        ...
        private (Mock<IRequestFactory>, IGameServersService) NewTestService()
        {
            var requestMock = new Mock<IRequestFactory>(MockBehavior.Strict);
        ...
        private static void MockAsyncResponse(Mock<IRequestFactory> mock, Task<string> response)
        {
            SetupAsyncMockRequest(mock).Returns(response);
        }

        private static void MockAsyncException(Mock<IRequestFactory> mock, Exception ex)
        {
            SetupAsyncMockRequest(mock).Throws(ex);
        }

        private static ISetup<IRequestFactory, Task<string>> SetupAsyncMockRequest(Mock<IRequestFactory> mock)
        {
