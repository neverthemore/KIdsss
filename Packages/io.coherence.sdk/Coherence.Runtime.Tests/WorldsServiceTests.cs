...
#pragma warning restore 0168
        ...
        public async Task Should_QueueCallbacks_When_RequestIsOnGoing()
        {
            ...
            requestMock.Setup(factory => factory.SendRequest(It.IsAny<string>(), It.IsAny<string>(),
                    ...
                    It.IsAny<Action<RequestResponse<string>>>()))
                .Callback<string, string, string, string, Dictionary<string, string>, string, string,
                    Action<RequestResponse<string>>>(async (s, s1, s2, arg3, arg4, arg5, arg6, arg7) =>
                    {
                        var response = new RequestResponse<string>()
                        {
                            Result = worldsResponse,
            ...
            bool receivedWorlds1 = false;

            worldService.FetchWorlds(list =>
            {
                if (list.Result.Count == 2)
                {
                    receivedWorlds1 = true;
                }
            });

            Assert.IsFalse(receivedWorlds1);
            ...
            bool receivedWorlds2 = false;

            worldService.FetchWorlds(list =>
            {
                if (list.Result.Count == 2)
                {
                    receivedWorlds2 = true;
                }
            });

            int counter = 0;
            while (!receivedWorlds1)
            {
                if (counter > 1000)
                {
            ...
            Assert.IsTrue(receivedWorlds1);
            Assert.IsTrue(receivedWorlds2);
        }
    }
}
