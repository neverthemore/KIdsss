namespace Coherence.Editor.Tests
{
    ...
    public class BakeUtilTest : CoherenceTest, IPostBuildCleanup
    {
        ...
        [Ignore("Long running test")]
#endif
        public IEnumerator Should_GenerateBakedScripts_When_Baking()
        {
            using(new TempPrefab(out var _))
            {
                ...
                var bakedFolder = Paths.defaultSchemaBakePath;
                var result = BakeUtil.Bake();
                Assert.That(result, "Baking passed");
                Assert.That(Directory.Exists(bakedFolder) && Directory.GetFiles(bakedFolder).Length > 0);
            }

            // Wait for domain reload to finish so that compile warnings from imported scripts can cause the test to
            // fail before it finishes.
            if (EditorApplication.isCompiling)
            {
                yield return new WaitForDomainReload();
            }
        ...
        {
            private const string DefaultPrefabName = "Test";
