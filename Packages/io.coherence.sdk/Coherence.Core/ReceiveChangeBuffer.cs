...
namespace Coherence.Core
{
    ...
    internal class ReceiveChangeBuffer
    {
        ...
        public void TakeUpdates(List<IncomingEntityUpdate> buffer, IReadOnlyCollection<Entity> resolvableEntities)
        {
            ...
            // Sort the changes by number of entity reference fields, putting the
            // changes with least references first, so there is a smaller chance of
            // pushing entities in the wrong order out of the core.
            ...
            // This is just a workaround to potentionally have better results in simple cases.
            SortChanges(buffer);
        }
        ...
        private static void SortChanges(List<IncomingEntityUpdate> buff)
        {
            buff.Sort((lhs, rhs) =>
            {
                var refDiff = GetNumberOfRefs(lhs) - GetNumberOfRefs(rhs);
                if (refDiff != 0)
                {
                    return refDiff;
                }

                return GetOperationPriority(lhs.Meta.Operation) - GetOperationPriority(rhs.Meta.Operation);
            });
        }
