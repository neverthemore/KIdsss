namespace Coherence.Editor
{
    ...
    public static class CoherenceSyncUtils
    {
        ...
        private static bool IsBindingUnrecoverable(Binding binding)
        {
            return binding == null || binding.Descriptor == null;
        }

        internal static int RemoveUnrecoverableBindings(CoherenceSync sync)
        {
            return RemoveBindings(sync, IsBindingUnrecoverable);
        }
        ...
        public static bool IsBindingValid(CoherenceSync sync, Binding binding, out string invalidReason)
        {
            ...
            if (!result)
            {
                invalidReason = binding.Descriptor.IsMethod
                    ? $"Binding is missing a valid Descriptor, did you change the signature or deleted {binding.Descriptor.Signature}?"
                    : $"Binding is missing a valid Descriptor, did you delete this member from {binding.UnityComponent.GetType().Name}?";
            }
