...
namespace Coherence.Editor
{
    ...
    internal class HubModuleManager : ScriptableSingleton<HubModuleManager>
    {
        ...
        public int Purge()
        {
            return moduleConnections.RemoveAll(mc => !mc.connection || !mc.module);
        }

        public void AssignAllModules(EditorWindow connection, AssignStrategy strategy = AssignStrategy.IgnoreAssiged)
        {
        ...
        private bool AssignModule(Type type, EditorWindow connection, out ModuleConnection moduleConnection, AssignStrategy strategy = AssignStrategy.IgnoreAssiged)
        {
            if (!connection)
            {
        ...
        private bool AssignModule(HubModule module, EditorWindow connection, out ModuleConnection moduleConnection, AssignStrategy strategy = AssignStrategy.IgnoreAssiged)
        {
            ...
            if (idx != -1 && strategy == AssignStrategy.IgnoreAssiged)
            {
                return false;
            }
        ...
        public ScriptableObject GetHost(HubModule module)
        {
            var mc = moduleConnections.FirstOrDefault(mc => mc.module == module);
            return mc != null ? mc.connection : default;
        }

        public IEnumerable<HubModule> GetActiveModules(ScriptableObject connection)
        {
            return moduleConnections.Where(mc => mc.connection == connection).Select(mc => mc.module);
        }
