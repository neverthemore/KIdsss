...
namespace Coherence.Editor
{
    ...
    internal class CoherenceHeader
    {
        ...
        {
            public static readonly GUIStyle slimHeader = new GUIStyle
            {
        ...
        private Vector2 logoSize = new Vector2(130f, 30f);
        private Vector2 inspectorLogoSize = new Vector2(130f, 20f);
        ...
        private Rect GetVersionLabelRect(float rectHeight)
        {
            ...
            var width = CoherenceHubLayout.Styles.VersionHeader.CalcSize(new GUIContent(CoherenceHub.info.ToString()))
                .x;
        ...
        private void DrawLoggedOutUi(Rect versionLabelRect)
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                if (GUI.Button(nextRect, guiContent, CoherenceHubLayout.Styles.BlueButton))
                {
                    ...
                    {
                        parent.Repaint();
                        CoherenceHub.FocusModule<OnlineModule>();
                    });
                }
            }
        }
        ...
        private void DrawLoggedInUi(Rect versionLabelRect)
        {
            using (new EditorGUILayout.HorizontalScope())
            {
                var filteredEmail = GetFilteredEmail();

                var labelContent = new GUIContent(filteredEmail);
                var btnContent = EditorGUIUtility.IconContent("d_dropdown@2x");
                var btnWidth = EditorStyles.label.CalcSize(btnContent).x;
                var maxLabelWidth = GetMaxLabelWidth(btnWidth);
                var labelStyle = CoherenceHubLayout.Styles.WhiteLabel;

                var rect = labelStyle.DrawLabel(labelContent, maxLabelWidth);

                TryToDrawVersionLabel(versionLabelRect, rect);

                if (GUILayout.Button(btnContent, EditorStyles.label, GUILayout.Width(btnWidth),
                GUILayout.Height(EditorGUIUtility.singleLineHeight)))
                {
                    var menu = new GenericMenu();
                    menu.AddItem(new GUIContent("Account"), false,
                    () => Application.OpenURL("https://coherence.io/dashboard/account"));
        ...
        private string GetFilteredEmail()
        {
            var email = ProjectSettings.instance.Email;
            var trimmedEmail = email.Remove(email.IndexOf("@"));
            var capitalizedEmail = char.ToUpper(trimmedEmail[0]) + trimmedEmail.Substring(1);
