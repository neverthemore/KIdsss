...
namespace Coherence.Editor.Toolkit
{
    ...
    internal class CoherenceInputEditor : BaseEditor
    {
        ...
        {
            public static readonly GUIContent reflectionModeWarning = EditorGUIUtility.TrTextContentWithIcon(
                $"Input requires Baked mode. Please make sure to enable it in the {nameof(CoherenceSync)} settings.",
                "Warning@2x");
        ...
        protected override void OnGUI()
        {
            ...
            if (!TryGetCoherenceSync(out CoherenceSync sync))
            {
                EditorGUILayout.LabelField(GUIContents.noCoherenceSyncWarning, ContentUtils.GUIStyles.miniLabel);
                return;
            }

            if (sync.usingReflection)
            {
                if (GUILayout.Button(GUIContents.reflectionModeWarning, ContentUtils.GUIStyles.miniLabel))
                {
                    HighlightBakedToggle(sync);
                }
        ...
        private void HighlightBakedToggle(CoherenceSync sync)
        {
            SerializedObject so = new SerializedObject(sync);
            SerializedProperty reflectionProperty = so.FindProperty(nameof(sync.usingReflection));
            Highlighter.Highlight("Inspector", reflectionProperty.propertyPath);
            bakedToggleHighlighted = true;
        }
