...
namespace Coherence.Editor.Toolkit
{
    ...
    internal partial class CoherenceSyncEditor : BaseEditor
    {
        ...
        private static partial class GUIContents
        {
            ...
            public static readonly GUIContent addEventListener = EditorGUIUtility.TrTextContent("Subscribe to...");
            ...
            public static readonly GUIContent savePrefabStage = EditorGUIUtility.TrTextContentWithIcon(
                $"The Stage is out-of-sync. Save the Prefab (if Auto Save is disabled), select the Prefab Asset in the Project Window, and come back to this GameObject.", "Info");
        ...
        private void DrawViewOnlyMode()
        {
            ...
            if (gameObjectStatus is { IsInstanceInScene: true } or { IsNestedInstanceInsideAnotherPrefab: true })
            {
                _ = EditorGUILayout.BeginVertical(GUI.skin.box);
        ...
        private bool IsEditingBindingsAllowed(CoherenceSync sync) =>
            CoherenceSyncUtils.IsEditingBindingsAllowed(sync, InteractionMode.UserAction) &&
            (gameObjectStatus.IsVariantAsset || gameObjectStatus.IsAsset ||
            gameObjectStatus.PrefabInstanceStatus == PrefabInstanceStatus.Connected);

        private void OnPerformedAction()
        {
            Repaint();
        }
        ...
        {
            var cantEdit = Application.isPlaying || ShouldDisableSyncProperty();
            ...
            EditorGUI.BeginDisabledGroup(cantEdit);
        ...
        private void DrawEventSelection()
        {
            ...
            _ = EditorGUILayout.BeginVertical(GUI.skin.box);
            //Draw 'Add events' section
            if (GUILayout.Button(GUIContents.addEventListener))
            {
        ...
        private void DrawUniqueness()
        {
            ...
            EditorGUI.BeginDisabledGroup(ShouldDisableSyncProperty());
        ...
        private void DrawUUID()
        {
            ...
            using (new EditorGUILayout.HorizontalScope())
            {
                ...
                // Prefab variants are considered prefab instances, but they are also prefab variants + asset, so we can check for
                // both.
                using var scope = new EditorGUI.DisabledScope(!gameObjectStatus.IsInstanceInScene ||
                                                              (gameObjectStatus.IsVariantAsset &&
            ...
            {
                EditorGUI.indentLevel++;
                using (new EditorGUI.DisabledScope(ShouldDisableSyncProperty() || Application.isPlaying))
                {
                    _ = EditorGUILayout.PropertyField(replacementStrategyProperty,
                        GUIContents.replacementStrategyLabel);
                }

                using (new EditorGUI.DisabledScope(ShouldDisableSyncProperty() || Application.isPlaying))
                {
                    _ = EditorGUILayout.PropertyField(unsyncedEntityPriorityProperty,
                        GUIContents.remoteEntityResolutionPriorityLabel);
                }
        ...
        {
            EditorGUI.BeginDisabledGroup(ShouldDisableSyncProperty() || Application.isPlaying);
        ...
        private void DrawTransferAuthority()
        {
            ...
            EditorGUI.BeginDisabledGroup(ShouldDisableSyncProperty() || Application.isPlaying ||
                                         isSimulationTypeServerSideWithClientInput);
            {
                using var change = new EditorGUI.ChangeCheckScope();
        ...
        private void DrawNetworkEntry()
        {
            ...
            else
            {
                ...
                if (gameObjectStatus.IsInPrefabStage)
                {
                    ...
                    EditorGUILayout.LabelField(GUIContents.savePrefabStage, style);

                    if (GUILayout.Button(GUIContents.selectPrefabAsset, GUILayout.Height(32)))
                    {
        ...
        {
            _ = EditorGUILayout.BeginHorizontal();
            EditorGUI.BeginDisabledGroup((gameObjectStatus.IsInstanceInScene && !gameObjectStatus.IsAsset) ||
        ...
        private bool ProcessIllegalHierarchy()
        {
            ...
            || gameObjectStatus.IsNestedInstanceInsideAnotherPrefab
            || gameObjectStatus.IsInstanceInScene
