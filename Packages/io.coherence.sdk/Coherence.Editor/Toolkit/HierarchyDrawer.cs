...
namespace Coherence.Editor.Toolkit
{
    ...
    internal static class HierarchyDrawer
    {
        private static class GUIContents
        {
            ...
            public static readonly GUIContent simulator = Icons.GetContent("Coherence.ConnectionType.Simulator", "Loaded through CoherenceSceneLoader with Simulator connection type.");
        ...
        private static readonly Dictionary<int, Component> hash = new Dictionary<int, Component>();
        private delegate int[] GetExpandedIDsCallback(object sceneHierarchyWindow);
                    ...
                    if (instanceID == scene.handle)
                    {
                        ...
                        if (CoherenceBridgeStore.TryGetBridge(scene, out var sceneBridge) && sceneBridge.Client != null)
                        {
                            ...
                            if (sceneBridge.IsConnected)
                            {
                                ...
                                GUI.Label(connTypeRect, sceneBridge.ConnectionType == ConnectionType.Simulator ? GUIContents.simulator : GUIContents.client, GUIStyle.none);
