...
namespace Coherence.Editor
{
    ...
    internal static class AssetUtils
    {
        ...
        public static bool DeleteFile([DisallowNull] AssetPath filePath)
        {
            ...
            if(!filePath.IsProjectRelativePath())
            {
                Debug.LogWarning($"Will not delete file at path '{filePath}', because it is not a project relative path. Use File.Delete instead to remove files located outside the project folder.");
                return false;
            }
        ...
        {
            var destinationIsInsideAssetsFolder = destinationPath.ToString().Contains("Assets/");

            if (Path.GetDirectoryName(destinationPath) is string destinationFolder)
            {
                if (destinationIsInsideAssetsFolder)
                {
            ...
            var isUnityAsset = sourcePath.ToString().Contains("Assets/") && AssetDatabase.LoadAssetAtPath<Object>(sourcePath) != null;

            if (!destinationIsInsideAssetsFolder || !isUnityAsset)
            {
