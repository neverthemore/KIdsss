...
namespace Coherence.Editor.ReplicationServer
{
    ...
    public static class EditorLauncher
    {
        ...
        public static ReplicationServerConfig CreateLocalWorldConfig()
        {
            ...
            var infiniteTimeout = projectSettings.UseToolkit && projectSettings.keepConnectionAlive;
        ...
        private class EditorConfigProvider : IConfigProvider
        {
            ...
            {
                var schemas = new List<string>(2);

                if (ProjectSettings.instance.UseToolkit)
                {
                ...
                {
                    schemas.AddRange(
                        ProjectSettings.instance.RuntimeSettings.extraSchemas.Where(s => s != null).Select(s => Path.GetFullPath(AssetDatabase.GetAssetPath(s)))
                );
                }
