...
namespace Coherence.Cloud
{
    ...
    public class LobbySession : IDisposableInternal
    {
        ...
        /// <summary>
        /// Callback that will be invoked when a Lobby its disposed and will no longer be usable. This can happen by manually
        /// leaving the Lobby, or logging out from your current user.
        /// </summary>
        ...
        public void LeaveLobby(Action<RequestResponse<bool>> onRequestFinished)
        {
            ...
            var pathParams = $"/{lobbyData.Id}/players/{authClient.UserId}";
        ...
        /// <param name="messages">List of messages to be sent to other Players.</param>
        /// <param name="targets">Optional list of Players to send the messages to. If left empty, it will be send to everyone.</param>
        public async Task<bool> SendMessageAsync(List<string> messages, List<Player> targets = null)
        {
            ThrowIfDisposed();
        ...
        {
            return requestFactory.GetRequestCooldown(lobbiesResolveEndpoint + $"/{lobbyData.Id}/players/{authClient.UserId}/attrs", "PATCH");
        }
        ...
        public void AddOrUpdateMyAttributes(List<CloudAttribute> attributes, Action<RequestResponse<bool>> onRequestFinished)
        {
            ...
            var pathParams = $"/{lobbyData.Id}/players/{authClient.UserId}/attrs";
        ...
        private void ParseLobbyData()
        {
            ...
            {
                if (player.UserId.Equals(authClient.UserId))
                {
                    myPlayer = player;
                }

                if (lobbyData.OwnerId == player.UserId)
                {
                    ownerPlayer = player;
                }
            }
        ...
        private void OnPlayerLeftHandler(string responseBody)
        {
            ...
            foreach (var player in lobbyData.Players)
            {
                if (player.UserId.Equals(response.PlayerId))
                {
        ...
        private void OnPlayerAttributesChangedHandler(string responseBody)
        {
            ...
            Player? playerWithChangedAttributes = null;
        ...
        private void OnLobbyOwnerChangedHandler(string responseBody)
        {
            ...
            lobbyData.OwnerId = newOwner.Value.UserId;
