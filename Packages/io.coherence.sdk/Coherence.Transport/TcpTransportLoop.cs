...
namespace Coherence.Transport
{
    ...
    internal class TcpTransportLoop
    {
        ...
        private readonly IDisposable clientDisposer;
        ...
        public Task Run()
        {
            ...
            {
                await RunSendLoopAsync();
            });

            return Task.WhenAll(receiveLoop, sendLoop);
        }
        ...
        private async Task RunReceiveLoopAsync()
        {
            ...
            try
            {
                ...
                {
                    (bool headerOk, int packetSize) = await ReceivePacketHeader(headerBuffer);
                    if (!headerOk)
                    {
                        return;
                    }

                    bool packetOk = await ReceivePacketPayload(packetSize);
                    if (!packetOk)
                    {
                        return;
                    }
        ...
        {
            if (await ReceiveFull(stream, headerBuffer, cancellationToken) != headerBuffer.Length)
            {
                return (false, 0);
            }
        ...
        private async Task<bool> ReceivePacketPayload(int packetSize)
        {
            ...
            if (await ReceiveFull(stream, packet, cancellationToken) != packetSize)
            {
                return false;
            }
        ...
        {
            byte[] headerPacket = CreateMagicAndRoomIDPacket();
        ...
        private void DisposeClient()
        {
            try
            {
                clientDisposer.Dispose();
            }
            catch (Exception exception)
            {
                logger.Debug("Client dispose error", ("exception", exception));
            }
        }

        private static async Task<int> ReceiveFull(Stream stream, byte[] buffer, CancellationToken token)
        {
            int totalRead = 0;
