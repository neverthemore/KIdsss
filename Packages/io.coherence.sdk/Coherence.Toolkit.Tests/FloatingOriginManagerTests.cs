...
namespace Coherence.Toolkit.Tests
{
    ...
    public class FloatingOriginManagerTests : CoherenceTest
    {
        ...
        {
            Assert.That(logger.GetLogLevelCount(LogLevel.Warning), Is.EqualTo(0), "There should be no untracked warnings.");
            Assert.That(logger.GetLogLevelCount(LogLevel.Error), Is.EqualTo(0), "There should be no untracked errors.");
        ...
        public void SetFloatingOrigin_WhenImpreciseRange_ShouldWarnOrThrow(bool useTranslate, double foX, double foY, double foZ, bool shouldWarn, bool shouldThrow)
        {
            ...
            if (shouldWarn)
            {
                Assert.That(logger.GetLogLevelCount(LogLevel.Warning), Is.EqualTo(1));
                logger.ClearLogLevelCount(LogLevel.Warning);
            }
        }
