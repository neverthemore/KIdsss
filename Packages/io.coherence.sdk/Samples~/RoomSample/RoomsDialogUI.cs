...
namespace Coherence.Samples.RoomsDialog
{
    ...
    public class RoomsDialogUI : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            {
                Debug.LogError($"'{nameof(CoherenceBridge)}.{nameof(CoherenceBridge.AutoLoginAsGuest)}' required.\n" +
                               $"Can be enabled via the inspector or by setting '{nameof(CoherenceBridge)}.{nameof(CoherenceBridge.AutoLoginAsGuest)}' to true.");

                windowContent.SetActive(false);
                errorText.text = $"'Auto Login As Guest' must be enabled in {nameof(CoherenceBridge)} for this sample.";
