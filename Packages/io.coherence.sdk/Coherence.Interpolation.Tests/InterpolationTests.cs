...
namespace Coherence.Interpolation.Tests
{
    ...
    public class InterpolationTests : CoherenceTest
    {
        ...
        private static readonly Sample<Vector3> Sample0 = new Sample<Vector3>(new Vector3(0, 1, 2), false, AbsoluteSimulationFrame.Invalid / InterpolationSettings.SimulationFramesPerSecond);
        private static readonly Sample<Vector3> SampleA = new Sample<Vector3>(new Vector3(1, 2, 3), false, 0);
        private static readonly Sample<Vector3> SampleAStopped = new Sample<Vector3>(new Vector3(1, 2, 3), true, 0);
        private static readonly Sample<Vector3> SampleB = new Sample<Vector3>(new Vector3(4, 5, 6), false, 1);
        private static readonly Sample<Vector3> SampleC = new Sample<Vector3>(new Vector3(10, 20, 30), false, 0);
        private static readonly Sample<Vector3> SampleD = new Sample<Vector3>(new Vector3(40, 50, 60), false, 10);

        private static readonly Sample<long> SampleALong = new Sample<long>(0, false, 0);
        private static readonly Sample<long> SampleBLong = new Sample<long>(long.MaxValue, false, 1);
        private static readonly Sample<long> SampleCLong = new Sample<long>(long.MinValue, false, 0);
        private static readonly Sample<long> SampleDLong = new Sample<long>(long.MaxValue / 2 + 50, false, 1);

        private static readonly Sample<float> SampleAFloat = new Sample<float>(0, false, 0);
        private static readonly Sample<float> SampleBFloat = new Sample<float>(float.MaxValue, false, 1);
        private static readonly Sample<float> SampleCFloat = new Sample<float>(float.MinValue, false, 0);
        private static readonly Sample<float> SampleDFloat = new Sample<float>(float.MaxValue / 2 + 50, false, 1);

        private static readonly Sample<double> SampleADouble = new Sample<double>(0, false, 0);
        private static readonly Sample<double> SampleBDouble = new Sample<double>(double.MaxValue, false, 1);
        private static readonly Sample<double> SampleCDouble = new Sample<double>(double.MinValue, false, 0);
        private static readonly Sample<double> SampleDDouble = new Sample<double>(double.MaxValue / 2 + 50, false, 1);
        ...
        private static SampleTestData<Vector3>[] linearInterpolationTestSource = {
            ...
            new(1f + BindingInterpolator<Vector3>.MaxOvershootAllowed, 1,
                SampleA.Value + (SampleB.Value - SampleA.Value) * (1f + BindingInterpolator<Vector3>.MaxOvershootAllowed), new []{SampleA, SampleB}),
        ...
        public void TestLinearInterpolation(SampleTestData<Vector3> testData)
        {
            ...
            {
                interpolator.AppendSample(sample.Value, sample.Stopped, sample.Frame, sample.Frame);
                interpolator.GetValueAt(testData.Time); // necessary to set IsStopped
            }
