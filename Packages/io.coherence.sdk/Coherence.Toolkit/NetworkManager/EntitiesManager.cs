    ...
    public class EntitiesManager : IEntitiesManager
    {
        ...
        internal virtual (NetworkEntityState, ComponentUpdates?, uint?, bool) SyncNetworkEntityState(ICoherenceSync sync)
        {
            ...
            if (FoundUnsyncedNetworkEntity(result))
            {
                ...
                // Since the sync is swapping states,
                // the old state can't still have a handle to the
                // sync it no longer represents.  TODO: refactor this
                // so there's only ever a one-way reference from
                // sync to state and not the other way around, too.
        ...
        private void SetClient(IClient client)
        {
            ...
            client.OnEntityDestroyed += OnRemoteNetworkEntityDestroyed;
        ...
        private NetworkEntityState CreateEntity(ICoherenceSync sync, string uuid)
        {
            ...
            bool isNestedPrefab = sync.IsChildFromSyncGroup() && !string.IsNullOrEmpty(uuid);
        ...
        private void UpdateNetworkedEntity(Entity id, IncomingEntityUpdate entityUpdate)
        {
            if (TryGetNetworkEntityState(id, out var state))
            {
                ...
                sync.ApplyNodeBindings();

                // Need to delay the connected entity changed callback until after all updates
                // applied.
                if (sync.Updater.ChangedConnection)
                {
                    sync.ConnectedEntityChanged(sync.Updater.NewConnection);
                }
        ...
        private void OnRemoteNetworkEntityDestroyed(Entity entityID, DestroyReason destroyReason)
        {
            if (entityID == Entity.InvalidRelative)
            {
                logger.Warning(Log.Warning.ToolkitEntitiesManagerRemoteEntityInvalid,
                    $"We can't delete an entity with ID 'default', this call to '{nameof(OnRemoteNetworkEntityDestroyed)}' will be ignored.",
                    ("context", this));
                return;
            }
            ...
            IncomingEntityUpdate entityUpdate, CoherenceSync.UnsyncedNetworkEntityPriority unsyncedNetworkEntityPriority)
        {
            ...
            bool isConnectionEntity = info.clientId.HasValue && info.connectionType.HasValue;
            CoherenceClientConnection connection = isConnectionEntity ? new CoherenceClientConnection(bridge, networkEntity, entityID,
        ...
        private void PostNetworkEntityCreationActions(NetworkEntityState state)
        {
            ...
            InstantiateDelayedEntitiesDependingOnThisParent(state.EntityID);
        }

        internal void InstantiateDelayedEntitiesDependingOnThisParent(Entity parentEntity)
        {
            if (!delayedUpdatesDependingOnParents.TryGetValue(parentEntity,
                    out var updatesDependingOnThisEntity))
            {
                return;
            }

            delayedUpdatesDependingOnParents.Remove(parentEntity);
