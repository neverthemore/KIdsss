...
namespace Coherence
{
    ...
    public static class SimulatorUtility
    {
        ...
        private static Dictionary<string, string> ArgumentsDict { get; set; } = new Dictionary<string, string>();
        ...
        public static Type SimulatorType
        {
            get
            {
                if (string.CompareOrdinal(GetArgument("--coherence-simulator-type"), SimulatorTypeWorldParameter) == 0)
                {
                    return Type.World;
                }
                else if (string.CompareOrdinal(GetArgument("--coherence-simulator-type"), SimulatorTypeRoomsParameter) == 0)
                {
                    return Type.Rooms;
                }
                else
        ...
        public static int Port => int.TryParse(GetArgument("--coherence-port"), out int port) ? port : 0;
        public static int RoomId => ushort.TryParse(GetArgument("--coherence-room-id"), out ushort id) ? id : -1;
        public static ulong UniqueRoomId => ulong.TryParse(GetArgument("--coherence-unique-room-id"), out ulong id) ? id : default;
        public static ulong WorldId => ulong.TryParse(GetArgument("--coherence-world-id"), out ulong id) ? id : 0;
        public static int HttpServerPort => int.TryParse(GetArgument("--coherence-http-server-port"), out int port) ? port : -1;
        ...
        public static List<string> RoomTags
        {
            get
            {
                ...
                string encodedS = GetArgument("--coherence-room-tags");
                if (string.IsNullOrEmpty(encodedS))
                {
                    return tags;
                }

                byte[] data = Convert.FromBase64String(encodedS);
        ...
        {
            ArgumentsDict.Add(arg, val);
        }

        private static string GetArgument(string arg)
        {
            return ArgumentsDict.TryGetValue(arg, out string value) ? value : null;
        }
    }
}
